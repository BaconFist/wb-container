...
function GetFolders( $resourceType, $currentFolder )
{
    ...
    if ($oCurrentFolder !== false)
    {
        while ( $sFile = readdir( $oCurrentFolder ) )
        {
            ...
            $isDot = (substr($aInfo['basename'], 0, 1) === '.');
            if ($isDot){continue;}
...
function GetFoldersAndFiles( $resourceType, $currentFolder )
{
    ...
    if ($oCurrentFolder !== false)
    {
        ...
        {
            $aInfo = pathinfo($sFile);
            $sExt = isset($aInfo['extension']) ? $aInfo['extension'] : '';
            $isDot = (substr($aInfo['basename'], 0, 1) === '.');
            if (\is_array($Config['AllowedExtensions'][$resourceType])){
                $sAllowedFileTypes = implode('|',$Config['AllowedExtensions'][$resourceType]);
            } else {
                $sAllowedFileTypes = preg_replace( '/\s*[,;\|#]\s*/','|',$Config['AllowedExtensions'][$resourceType]);
            }
            $isValidFile = (preg_match('/' . $sAllowedFileTypes . '$/i', $sExt));
            if ($isDot){continue;}
