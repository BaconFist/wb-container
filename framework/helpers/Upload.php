...
// libjpeg: recoverable error: Corrupt JPEG data: premature end of data segmen
// 7.1.0     The default of gd.jpeg_ignore_warning has been changed from 0 to 1.
...
class Upload {
    ...
    /**
     * Set this variable true to allow overwriting of an existing file
     *
    ...
    public function upload($file, $lang = 'en_GB') {
        ...
        if (empty($this->log)) {
            $this->log .= sprintf('<b>system information</b><br />');
            ...
            $this->log .= sprintf('-&nbsp;class version           : ' . $this->version . '<br />');
            $this->log .= sprintf('-&nbsp;operating system        : ' . PHP_OS . '<br />');
            $this->log .= sprintf('-&nbsp;PHP version             : ' . PHP_VERSION . '<br />');
            $this->log .= sprintf('-&nbsp;GD version              : ' . $gd . '<br />');
            $this->log .= sprintf('-&nbsp;supported image types   : ' . (!empty($supported) ? $supported : 'none') . '<br />');
            $this->log .= sprintf('-&nbsp;open_basedir            : ' . (!empty($open_basedir) ? $open_basedir : 'no restriction') . '<br />');
            $this->log .= sprintf('-&nbsp;upload_max_filesize     : ' . $this->file_max_size_raw . ' (' . $this->file_max_size . ' bytes)<br />');
            $this->log .= sprintf('-&nbsp;language                : ' . $this->lang . '<br />');
        }
    ...
    public function function_enabled($func) {
        ...
        return (is_callable($func) && !in_array($func, $disabled) && !in_array($func, $blacklist));
    }
    ...
    public function imagecopymergealpha(&$dst_im, &$src_im, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h, $pct = 0) {
      ...
      public function getExifData($FileSrcPathname){
          $exif = false;
          if (version_compare(PHP_VERSION,'7.2.0','>=')) {
              try {
                  if (\is_file($FileSrcPathname)){
                      $exif = @\exif_read_data($FileSrcPathname, 'ANY_TAG');
                  }
              } catch (\Exception $ex) {
                $exif = false;
              }
        ...
        if ($this->processed) {
            ...
            if (!$return_mode) {
                if (!$this->file_auto_rename) {
                    ...
                    $ext = '';
                    $ext = '.'.($file_src_name_ext ? $file_src_name_ext : $this->file_src_name_ext); // dw changed 20200514
                    ...
                    $cpt = 1;
                    while (is_readable($this->file_dst_pathname)) {
                        $this->file_dst_name_body = str_replace($ext,'',$body) . '_' . $cpt . $ext;
                        $this->file_dst_name = $this->file_dst_name_body . (!empty($this->file_dst_name_ext) ? '.' . $this->file_dst_name_ext : '');
                        $cpt++;
                        $this->file_dst_pathname = $this->file_dst_path . $this->file_dst_name;
                    }
                ...
                if ($this->file_overwrite) {
                    ...
                    if (is_readable($this->file_dst_pathname)) {
                        $this->processed = false;
...
// dw20171213
            try {
                if (!is_file($this->file_src_pathname)){throw new \Exception (' file not found');}
                $orientation = 1;
                if (preg_match('@\x12\x01\x03\x00\x01\x00\x00\x00(.)\x00\x00\x00@', file_get_contents($this->file_src_pathname), $matches)) {
                    $orientation = ord($matches[1]);
                }
                $exif = $this->getExifData($this->file_src_pathname);
                } catch (Exception $ex) {
                  $exif = false;
                }
...
// end image_resize
        ...
        if ($return_mode){ return $return_content;}else{ return null;}
    }
