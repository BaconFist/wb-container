...
trait GdprTrait
{
    ...
    public function checkGdpr($iFlags=0)
    {
        $bRetval = false;
        if (($iFlags > 0)){
            $aRequestVars = $this->request_vars;
            $bRetval = ($aRequestVars['data_protection'] ?? false);
        }
        return $bRetval;
    }

    public function GdprSettings(){
        if ($sSettings = ParentList::gdprSettings($iFlags=0))
        {
            $aSettings = ParentList::unserialize($sSettings);
            $iUseDataProtection = $aSettings['use_data_protection'];
        }
        return $iUseDataProtection;
    }

    public function getGdprFlags($iFlags=0)
    {
        $aRetval = ['enabled_loginform','enable_signup','enabled_lostpassword'];
        $oReg = WbAdaptor::getInstance();
        $oApp = $this->oReg->getApplication();
        $bRetval = $oApp->bit_isset($this->GdprSettings(),$iFlags);
        return $bRetval;
    }
