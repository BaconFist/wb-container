 ...
 * @link https://phpthumb.gxdlabs.com
...
class GdThumb extends ThumbBase
{
     ...
     */
    public function __construct ($fileName, array $options = [], $isDataStream = false)
    {
        ...
        switch ($this->format)
        {
            ...
            case 'JPEG':
            case 'JPG':
                $this->oldImage = $this->imagecreatefromjpegexif($fileName);
                break;
            case 'PNG':
...
*/
    ...
    public function imagecreatefromjpegexif($filename)
    {
        $img = \imagecreatefromjpeg($filename);
        $exifData = (@\exif_read_data($filename) ?? null);
        if ($img && $exifData && isset($exifData['IFD0']))
        {
            ($ort = $exifData['IFD0']['Orientation']);
            if ($ort == 6 || $ort == 5)
    ...
    public function resize ($maxWidth = 0, $maxHeight = 0)
    {
        ...
        // make sure we're not exceeding our image size if we're not supposed to
                ...
                {
                    $this->maxHeight    = (((int)$maxHeight > $this->currentDimensions['height']) ? $this->currentDimensions['height'] : (int)$maxHeight);
                    $this->maxWidth     = (((int)$maxWidth  > $this->currentDimensions['width'])  ? $this->currentDimensions['width']  : (int)$maxWidth);
                }
                else
                {
