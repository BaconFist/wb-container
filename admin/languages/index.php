...
use bin\requester\HttpRequester;
    ...
    $oTrans->enableAddon($sModuleName.'\\addons');
...
/*
    $sAddonBackUrl  = WB_URL.'/'.ADMIN_DIRECTORY.'/modules/index.php'.($show_block ? '?advanced='.$show_block : '');
    throw new \Exception ($oTrans->HEADING_UPGRADE_LANGUAGE."\n".$oTrans->MESSAGE_GENERIC_WEBSITE_UNDER_CONSTRUCTION);
*/
...
//        $aDebug['details'][$sAddonIdKey] = $aAddon['addon_id'].' ['.$aAddon['directory'].']';
        $sAddsonsPath = WB_PATH.$sAddonAppDir;
        if (is_readable($sAddsonsPath . '/'.$aAddon['directory'].'.php')) {
